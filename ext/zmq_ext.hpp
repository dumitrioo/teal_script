...
namespace scfx {
    ...
    class zmq_ext: public extension_interface {
    public:
        zmq_ext() = default;
        ...
        void register_runtime(runtime_interface *rt) override {
            ...
            rt->add_function("zmq_errno", SCFXFUN(/*fname*/, /*args*/) { return zmq_errno(); });
            rt->add_function("zmq_errstr", SCFXFUN(/*fname*/, /*args*/) { return zmq_strerror(zmq_errno()); });
            rt->add_function("zmq_err_to_str", SCFXFUN(/*fname*/, args) { return zmq_strerror(args[0].cast_num_to_num<int>()); });
            rt->add_function("zmq_ctx_new", SCFXFUN(/*fname*/, /*args*/) { return zmq_ctx_new(); });
            rt->add_function("zmq_ctx_term", SCFXFUN(/*fname*/, args) { return zmq_ctx_term(args[0].as_ptr()); });
            rt->add_function("zmq_ctx_shutdown", SCFXFUN(/*fname*/, args) { return zmq_ctx_shutdown(args[0].as_ptr()); });
            rt->add_function("zmq_ctx_set", SCFXFUN(/*fname*/, args) { return zmq_ctx_set(args[0].as_ptr(), args[1].cast_num_to_num<int>(), args[2].cast_num_to_num<int>()); });
            rt->add_function("zmq_ctx_get", SCFXFUN(/*fname*/, args) { return zmq_ctx_get(args[0].as_ptr(), args[1].cast_num_to_num<int>()); });
            rt->add_function("zmq_socket", SCFXFUN(/*fname*/, args) { return zmq_socket(args[0].as_ptr(), args[1].cast_num_to_num<int>()); });
            rt->add_function("zmq_bind", SCFXFUN(/*fname*/, args) { return zmq_bind(args[0].as_ptr(), args[1].cast_to_string().c_str()); });
            rt->add_function("zmq_connect", SCFXFUN(/*fname*/, args) { return zmq_connect(args[0].as_ptr(), args[1].cast_to_string().c_str()); });

            rt->add_function("zmq_setsockopt", SCFXFUN(/*fname*/, args) {
            ...
            rt->add_function("zmq_z85_encode", SCFXFUN(/*fname*/, args) {
            ...
            rt->add_function("zmq_z85_decode", SCFXFUN(/*fname*/, args) {
                SCFX_CHCK_FUN_PARMS_NUM_BETWEEN(1, 1)
            ...
            rt->add_function("zmq_send", SCFXFUN(/*fname*/, args) {
            ...
            rt->add_function("zmq_send_more", SCFXFUN(/*fname*/, args) {
                SCFX_CHCK_FUN_PARMS_NUM_BETWEEN(2, 2)
                std::vector<std::uint8_t> buf{args[1].cast_to_byte_array()};
                return zmq_send(args[0].as_ptr(), buf.data(), buf.size(), ZMQ_MORE);
            });
            rt->add_function("zmq_recv", SCFXFUN(/*fname*/, args) {
