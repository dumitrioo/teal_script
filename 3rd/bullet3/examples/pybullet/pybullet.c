	...
	if (b3CanSubmitCommand(sm))
	{
		...
		if (statusType == CMD_CAMERA_IMAGE_COMPLETED)
		{
			...
			// TODO(hellojas): error handling if image size is 0
			{
				...
				memcpy(PyArray_DATA((const PyArrayObject *)pyRGB), imageData.m_rgbColorData,
					   imageData.m_pixelHeight * imageData.m_pixelWidth * bytesPerPixel);
				memcpy(PyArray_DATA((const PyArrayObject *)pyDep), imageData.m_depthValues,
					   imageData.m_pixelHeight * imageData.m_pixelWidth * sizeof(float));
				memcpy(PyArray_DATA((const PyArrayObject *)pySeg), imageData.m_segmentationMaskValues,
