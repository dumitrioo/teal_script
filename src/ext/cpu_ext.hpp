...
namespace scfx {
    ...
    public:
        ...
        void register_runtime(runtime_interface *rt) override {
            ...
            rt->add_function("perf_stat", SCFXFUN(/*fname*/, /*args*/) {
                return scfx::valbox{std::make_shared<cpu_pc<double>>(), "perf_stat"};
            });
            rt->add_method("perf_stat", "start", SCFXFUN(/*fname*/, args) {
                return SCFXTHIS(std::shared_ptr<cpu_pc<double>>)->start();
            });
            rt->add_method("perf_stat", "stop", SCFXFUN(/*fname*/, args) {
                SCFXTHIS(std::shared_ptr<cpu_pc<double>>)->stop();
                return 0;
            });
            rt->add_method("perf_stat", "cpu_load", SCFXFUN(/*fname*/, args) {
                return SCFXTHIS(std::shared_ptr<cpu_pc<double>>)->cpu_load();
            });
            rt->add_method("perf_stat", "self_cpu_consumption", SCFXFUN(/*fname*/, args) {
