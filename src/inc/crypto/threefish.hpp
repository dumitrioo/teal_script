...
namespace teal {
    ...
    namespace crypt {
        ...
        template<std::size_t STATE_SIZE = 512>
        class threefish {
            static_assert(STATE_SIZE == 256 || STATE_SIZE == 512 || STATE_SIZE == 1024, "cipher class template parameter must be 256/512/1024");
            static constexpr std::uint64_t key_schedule_const = 0x1BD11BDAA9FC1A22LL;
            static constexpr int SKEIN_MAX_STATE_WORDS = 16;

        public:
            threefish() = default;
            threefish(void const *key, void const *iv) { init(key, iv); }
            threefish(threefish const &) = default;
            threefish &operator=(threefish const &) = default;
            threefish(threefish &&) = default;
            threefish &operator=(threefish &&) = default;
            ~threefish() = default;
            ...
            static constexpr std::size_t key_length() {
                return STATE_SIZE / 8;
            }

            static constexpr std::size_t iv_length() {
                return 16;
            }

            bool init(void const *key_data, void const *tweak_data) {
                int key_words = STATE_SIZE / 64;
                int i;
                uint64_t parity = key_schedule_const;

                if(tweak_data) {
                    tweak_[0] = ((std::uint64_t const *)tweak_data)[0];
                    tweak_[1] = ((std::uint64_t const *)tweak_data)[1];
                    tweak_[2] = ((std::uint64_t const *)tweak_data)[0] ^ ((std::uint64_t const *)tweak_data)[1];
                } else {
                    tweak_[0] = 0;
                    tweak_[1] = 0;
                    tweak_[2] = 0;
                ...
                for (i = 0; i < key_words; i++) {
                    key_[i] = ((std::uint64_t const *)key_data)[i];
                    parity ^= ((std::uint64_t const *)key_data)[i];
                }
                key_[i] = parity;
                return true;
            }

            void encrypt_block(void const *in, void *out) {
                std::uint64_t plain[SKEIN_MAX_STATE_WORDS];        /* max number of words*/
                std::uint64_t cipher[SKEIN_MAX_STATE_WORDS];

                skein_get64_lsb_first(plain, in, STATE_SIZE / 64);   /* bytes to words */
                threefishEncryptBlockWords(plain, cipher);
                skein_put64_lsb_first(out, cipher, STATE_SIZE / 8);  /* words to bytes */
            }

            void decrypt_block(void const *in, void *out) {
                uint64_t plain[SKEIN_MAX_STATE_WORDS];        /* max number of words*/
                uint64_t cipher[SKEIN_MAX_STATE_WORDS];

                skein_get64_lsb_first(cipher, in, STATE_SIZE / 64);  /* bytes to words */
                threefishDecryptBlockWords(cipher, plain);
                skein_put64_lsb_first(out, plain, STATE_SIZE / 8);   /* words to bytes */
            }

        private:
            static void skein_put64_lsb_first(void *dst08, void const *src64, size_t bCnt) {
                if constexpr (sys_util::little_endian()) {
                    std::memcpy(dst08, src64, bCnt);
                } else {
                    size_t n;
                    for(n = 0; n < bCnt; n++) {
                        ((std::uint8_t *)dst08)[n] = (((std::uint64_t const*)src64)[n>>3] >> (8 * (n & 7)));
                    }
                }
            }

            static void skein_get64_lsb_first(void *dst64, void const *src08, size_t wCnt) {
                if constexpr (sys_util::little_endian()) {
                    std::memcpy(dst64, src08, 8 * wCnt);
                } else {
                    size_t n;
                    for(n = 0; n < 8 * wCnt; n += 8)
                        ((std::uint64_t *)dst64)[n/8] = (((std::uint64_t) ((std::uint8_t *)src08)[n])) +
                                   (((std::uint64_t) ((std::uint8_t *)src08)[n + 1]) <<  8) +
                                   (((std::uint64_t) ((std::uint8_t *)src08)[n + 2]) << 16) +
                                   (((std::uint64_t) ((std::uint8_t *)src08)[n + 3]) << 24) +
                                   (((std::uint64_t) ((std::uint8_t *)src08)[n + 4]) << 32) +
                                   (((std::uint64_t) ((std::uint8_t *)src08)[n + 5]) << 40) +
                                   (((std::uint64_t) ((std::uint8_t *)src08)[n + 6]) << 48) +
                                   (((std::uint64_t) ((std::uint8_t *)src08)[n + 7]) << 56) ;
                }
            }

            void threefishEncryptBlockWords(uint64_t* in, uint64_t* out) const {
                if constexpr (STATE_SIZE == 256) {
                    threefishEncrypt256(in, out);
                } else if constexpr (STATE_SIZE == 512) {
                    encrypt_block_512(in, out);
                } else if constexpr (STATE_SIZE == 1024) {
                    threefishEncrypt1024(in, out);
                }
            }

            void threefishDecryptBlockWords(uint64_t* in, uint64_t* out) const {
                if constexpr (STATE_SIZE == 256) {
                    threefishDecrypt256(in, out);
                } else if constexpr (STATE_SIZE == 512) {
                    decrypt_block_512(in, out);
                } else if constexpr (STATE_SIZE == 1024) {
                    threefishDecrypt1024(in, out);
                }
            }

            void threefishEncrypt256(uint64_t* input, uint64_t* output) const {

                uint64_t b0 = input[0], b1 = input[1],
                  b2 = input[2], b3 = input[3];
                uint64_t k0 = key_[0], k1 = key_[1],
                  k2 = key_[2], k3 = key_[3],
                  k4 = key_[4];
                uint64_t t0 = tweak_[0], t1 = tweak_[1],
                  t2 = tweak_[2];
            ...
            void threefishDecrypt256(uint64_t* input, uint64_t* output) const
              {
                uint64_t b0 = input[0], b1 = input[1],
                  b2 = input[2], b3 = input[3];
                uint64_t k0 = key_[0], k1 = key_[1],
            ...
            void encrypt_block_512(void const *plain, void *cipher) const {
                uint64_t const *input{(uint64_t const *)plain};
            ...
            void decrypt_block_512(void const *cipher, void *plain) const {
                uint64_t const *input{(uint64_t const *)cipher};
                uint64_t *output{(uint64_t *)plain};
                uint64_t b0 = input[0], b1 = input[1],
                  b2 = input[2], b3 = input[3],
                  b4 = input[4], b5 = input[5],
                  b6 = input[6], b7 = input[7];
            ...
            void threefishEncrypt1024(uint64_t* input, uint64_t* output) const
                    {
                ...
                uint64_t k0 = key_[0], k1 = key_[1],
                  ...
                  k6 = key_[6], k7 = key_[7],
                  k8 = key_[8], k9 = key_[9],
                  k10 = key_[10], k11 = key_[11],
                  k12 = key_[12], k13 = key_[13],
                  k14 = key_[14], k15 = key_[15],
                  k16 = key_[16];
                uint64_t t0 = tweak_[0], t1 = tweak_[1],
                  t2 = tweak_[2];
            ...
            void threefishDecrypt1024(uint64_t* input, uint64_t* output) const
            {
        ...
        private:
            std::uint64_t key_[SKEIN_MAX_STATE_WORDS + 1];
            std::uint64_t tweak_[3];
