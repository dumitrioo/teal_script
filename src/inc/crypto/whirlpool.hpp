...
namespace teal {
    ...
    namespace crypt {
        ...
        public:
            ...
            void reset() {
                for(std::size_t i = 0; i < 8; i++) { h_[i] = 0ULL; }
                size_ = 0;
                totalSize_ = 0;
            }

            static constexpr std::size_t digest_size() {
                return 64;
            }

            void compute(const void *data, size_t length, uint8_t *digest) {
                reset();
                update(data, length);
                final(digest);
            }

            std::array<std::uint8_t, 64> compute(const void *data, size_t length) {
                std::array<std::uint8_t, 64> res{};
                compute(data, length, &res[0]);
                return res;
            }

            std::array<std::uint8_t, 64> compute(std::vector<std::uint8_t> const &data) {
                return compute(data.data(), data.size());
            }

            std::array<std::uint8_t, 64> compute(std::string const &data) {
                return compute(data.data(), data.size());
            }
        ...
        private:
            ...
            void final(std::uint8_t *digest) {
                ...
                if(size_ < 32) {
                    paddingSize = 32 - size_;
                } else {
                    paddingSize = 64 + 32 - size_;
                }

                update(padding_, paddingSize);

                x_[4] = 0;
                x_[5] = 0;
                x_[6] = 0;
                x_[7] = bit_util::hnswap<std::uint64_t>{totalSize}.val;
        ...
        private:
            static inline const std::uint8_t padding_[64] {
            ...
            static inline const std::uint64_t rc_[10] {
            ...
            static inline const std::uint64_t t_[256] {
            ...
            void rho(std::uint64_t &b, std::uint64_t *a, std::uint64_t n, std::uint64_t c) {
                b = t_[(a[n] >> 56) & 0xFFULL];
                b ^= teal::bit_util::ror<std::uint64_t>(t_[(a[(n + 7) % 8] >> 48) & 0xFFULL], 8).val;
                b ^= teal::bit_util::ror<std::uint64_t>(t_[(a[(n + 6) % 8] >> 40) & 0xFFULL], 16).val;
                b ^= teal::bit_util::ror<std::uint64_t>(t_[(a[(n + 5) % 8] >> 32) & 0xFFULL], 24).val;
                b ^= teal::bit_util::ror<std::uint64_t>(t_[(a[(n + 4) % 8] >> 24) & 0xFFULL], 32).val;
                b ^= teal::bit_util::ror<std::uint64_t>(t_[(a[(n + 3) % 8] >> 16) & 0xFFULL], 40).val;
                b ^= teal::bit_util::ror<std::uint64_t>(t_[(a[(n + 2) % 8] >> 8) & 0xFFULL], 48).val;
                b ^= teal::bit_util::ror<std::uint64_t>(t_[a[(n + 1) % 8] & 0xFFULL], 56).val;
                b ^= c;
            }

        private:
            union {
                std::uint8_t digest_[64]{};
                std::uint64_t h_[8];
            };
